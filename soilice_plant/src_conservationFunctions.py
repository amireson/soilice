...
@njit(inline='always')
def Richards(t,psif,psie,T,z,dz,pars,const,opts,nz,upperBC,E_PT,E_PS):
    ...
    # Source term for root water uptake:
    sv=rootUptake(E_PT,psie,T,z,dz,pars)
    E_AT=np.sum(-sv*dz)
    # Source term for soil evaporation
    s_AS=soilEvaporation(E_PS,psie,T,dz,pars)
    sv[0]+=s_AS
    E_AS=-s_AS*dz[0]

    # continuity
    dthetaTdt=-(q[1:]-q[:-1])/dz+sv
    dpsiedt=1/C*dthetaTdt
    
    return dthetaTdt,dpsiedt,q,sv,E_AT,E_AS

@njit(inline='always')
def heatbalanceFun(t,psie,psif,T,TTop,TBot,jTopAdv,jTopBC,jBotBC,dz,pars,const,opts,nz,dthetaTdt,q,sv):
    ...
    # Source terms:
    su=sv*const['cp_liq']*const['rho_liq']*T

    fluxDiv=-(j[1:]-j[:-1])/dz + su
    ...
    # Evapotranspiration energy flux associated with sensible heat of liquid water:
    jE=np.sum(-su*dz)
    
    return dTdt,j,jE

@njit(inline='always')
def ODEfunCall(t,DV,upperBC,TTop,TBot,TInf,jTopBC,jBotBC,E_PT,E_PS,z,dz,pars,const,opts,nz):
    ...
    if opts['simulateFlow']:
        dthetaTdt,dpsiedt,q,sv,E_AT,E_AS=Richards(t,psif,psie,T,z,dz,pars,const,opts,nz,upperBC,E_PT,E_PS)
    else:
    ...
    if opts['simulateTransport']:
        ...
        dTdt,j,jE=heatbalanceFun(t,psie,psif,T,TTop,TBot,jTopAdv,jTopBC,jBotBC,dz,pars,const,opts,nz,dthetaTdt,q,sv)
    else:
        dTdt=np.zeros(nz)
        j=np.zeros(nz+1)
        jE=0.
