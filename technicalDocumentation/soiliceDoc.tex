...
which may then be solved by replacing the derivative terms on the right hand side with the net fluxes. Note, in Ireson et al. (2026) these derivative terms are expanded, and the equations are further simplified. Here, we do not take this extra step, since the soilice model calculates $\partial u/\partial t$ and $\partial m/\partial t$ directly, and those values are substituted directly into Equations \ref{eq: unfrozen_dT} and \ref{eq: frozen_dT}. Hence our governing equations are consistent with those presented in Ireson et al. (2026).
