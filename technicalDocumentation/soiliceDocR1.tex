...
\frac{\partial m}{\partial t}=\frac{\partial m_l}{\partial t}+\frac{\partial m_i}{\partial t}=-\rho_l \nabla\cdot \mathbf{q}_l
\end{equation}
...
\frac{\partial u_{si}}{\partial t}+
\frac{\partial u_{sl}}{\partial t}+
\frac{\partial u_{ss}}{\partial t}+
\frac{\partial u_{li}}{\partial t}=
-\nabla\cdot (\mathbf{j}_a+\mathbf{j}_c+\mathbf{j}_o)
\end{equation}

where $\mathbf{q}_l$ (m s$^{-1}$) is the flux of liquid water and $\mathbf{j}_a$, $\mathbf{j}_c$ and $\mathbf{j}_o$ (J m$^{-2}$ s$^{-1}$) are the advective, conductive, and other heat flux components. Expanding the left hand side of the energy balance equation gives
...
\begin{equation}
	\begin{array}{cc}
\displaystyle \frac{\partial m_l}{\partial t}=\frac{\partial m}{\partial t};
& 
\displaystyle \frac{\partial m_i}{\partial t}=0
	\end{array}
\end{equation}
%
...
\frac{\partial T}{\partial t}
=
\frac{\displaystyle\frac{\partial u}{\partial t}
-c_{pl}T
...
\begin{equation}
\frac{\partial m_l}{\partial t}=\rho_lF'(T)\frac{\partial T}{\partial t}
\end{equation}

and

\begin{equation}
\frac{\partial m_i}{\partial t}=\frac{\partial m}{\partial t}-\frac{\partial m_l}{\partial t}
=
\frac{\partial m}{\partial t}-\rho_lF'(m,T)\frac{\partial T}{\partial t}
\end{equation}
...
which may then be solved by replacing the derivative terms on the right hand side with the net fluxes. Note, in Ireson et al. (2026 in prep) these derivative terms are expanded, and the equations are further simplified. Here, we do not take this extra step, since the soilice model calculates $\partial u/\partial t$ and $\partial m/\partial t$ directly, and those values are substituted directly into Equations \ref{eq: unfrozen_dT} and \ref{eq: frozen_dT}. Hence our governing equations are consistent with those presented in Ireson et al. (2026 in prep).

\section{Finite difference formulation}

For a one-dimensional vertical model, with $z$ (m) representing the depth below the ground surface, it is good practice to use a block-centred grid, where $z_n$ denotes the midpoint of a cell, and $z_{n-1/2}$ and $z_{n+1/2}$ are the depths of the top and bottom faces of the cell, respectively. Consider that the profile is discretized into $N$ cells, with $N+1$ faces. Using the method of lines we reduce our system of two PDEs to a system of $2N$ ODEs, as follows. Considering a single control volume at depth index $n$, we can write
...
where $\nabla\cdot \mathbf{q}_n$ and $\nabla \cdot \mathbf{j}_n$ are the net fluxes of mass and energy into the control volume. Expressing these equations with total mass, $m$ and temperature $T$ as the dependent variables we have 
...
We have here a system of ordinary differential equations which we solve using an ODE solver using the method described in Ireson et al (2023).
...
$q$ & Applied uniform flux for transport only & 0 & m s $^{-1}$ \\
$\text{impedence}$ & Impedence parameter for frozen $K$ & 0 & (-) \\
\hline
		\end{tabular}
...
\section{Source/sink terms}

Source and sink terms are useful in soilice particular to represent plant root water uptake and lateral input/output of mass and energy. This is included by a simple modification to the conservation equations. The original conservation equations, are given by

\begin{equation}
\frac{\partial \psi_e}{\partial t}=-\frac{1}{C(\psi_e)}\nabla\cdot\mathbf{q}_l
\end{equation}

For frozen conditions (i.e., $\psi_e>\psi_f$) we have

\begin{equation}
\frac{\partial T}{\partial t} = \frac{-\nabla\cdot\mathbf{j}+(c_{pi}T-\lambda_f)(\rho_l\nabla\cdot\mathbf{q}_l)}{c_{pb}+\rho_lF'(T)((c_{pl}-c_{pi})T+\lambda_f)}
\end{equation}

and for unfrozen conditions (i.e., $\psi_e\leq\psi_f$) we have

\begin{equation}
\frac{\partial T}{\partial t} = \frac{-\nabla\cdot\mathbf{j}+c_{pl}T(\rho_l\nabla\cdot\mathbf{q}_l)}{c_{pb}}
\end{equation}

The source terms are added to the flux divergence terms. Hence we replace $-\nabla\cdot\mathbf{q}_l$ with $-\nabla\cdot\mathbf{q}_l + s_v$ and we replace $-\nabla\cdot\mathbf{j}$ with $-\nabla\cdot\mathbf{j}+s_u$, where $s_v$ (m$^3$ m$^{-3}$ s$^{-1}$) is the volumetric source term for liquid water (i.e. positive for addition of water) and $s_u$ (J m$^{-3}$ s$^{-1}$) is the source term for energy. Hence, the governing equations become

\begin{equation}
\frac{\partial \psi_e}{\partial t}=-\frac{1}{C(\psi_e)}(\nabla\cdot\mathbf{q}_l-s_v)
\end{equation}

For frozen conditions (i.e $\psi_e>\psi_f$) we have

\begin{equation}
\frac{\partial T}{\partial t} = \frac{-\nabla\cdot\mathbf{j}+s_u+(c_{pi}T-\lambda_f)\rho_l(\nabla\cdot\mathbf{q}_l-s_v)}{c_{pb}+\rho_lF'(T)((c_{pl}-c_{pi})T+\lambda_f)}
\end{equation}

and for unfrozen conditions (i.e. $\psi_e\leq\psi_f$) we have

\begin{equation} \label{eq: unfrozEbalSource}
\frac{\partial T}{\partial t} = \frac{-\nabla\cdot\mathbf{j}+s_u+c_{pl}T\rho_l(\nabla\cdot\mathbf{q}_l-s_v)}{c_{pb}}
\end{equation}

\section{Plant water uptake}

To understand how plant water uptake is represented in soilice, first consider the condition of zero water stress. Here we have an unstressed transpiration flux of $E_{TU}$ (m s$^{-1}$), all of which must be supplied by water uptake from the root system. This is distributed over the root zone as a volumetric source term, which is negative for plant water uptake, where 

\begin{equation}
	s_{v,i}=-E_{TU} w_i
\end{equation}

where $w_i$ (m$^{-1}$) is the weighting term for each cell, and is defined such that

\begin{equation}
\sum_{i=1}^N w_i dz_i = 1
\end{equation}

Note that the quantity $-s_{v,i}dz_i$ is the transpiration flux that is supplied by cell $i$.

If we had a completely uniform root distribution we would have $w_i=1/z_n$ (note that based on how the grid is defined, $z_n=\sum_{i=1}^N dz_i$). Consider that we have some root density function, $g(z)$ (kg m$^{-3}$). This function defines how the density varies with depth below ground, such that in any given cell, $i$, we have a root mass of $g(z_i)dx.dy.dz_i$, and the total mass of roots in the profile is $\sum_{i=1}^Ng(z_i)dx.dy.dz_i$. Now the relative amount of root mass in any given cell, $g_r(z_i)$ (kg kg$^{-1}$) will be given by

\begin{equation}
g_r(z_i)=\frac{g(z_i)dx\,dy\,dz_i}{\sum_{j=1}^N g(z_j) dx\,dy\,dz_j}
=\frac{g(z_i)dz_i}{\sum_{j=1}^N g(z_j)dz_j}
\end{equation}

and we find the associated weighting term based on 

\begin{equation}
w_i=\frac{g_r(z_i)}{dz_i}=\frac{g(z_i)}{\sum_{j=1}^N g(z_j) dz_j}
\end{equation}

Now, using this to calculate $s_{v,i}$ above we will see that

\begin{equation}
E_{TU}=\sum_{i=1}^N-s_{v,i}dz_i
\end{equation}

In the case where we have water stress, we need to define potential transpiration, $E_{TP}$ (m s$^{-1}$), and actual transpiration, $E_{TA}$ (m s$^{-1}$), such that $E_{TA}\leq E_{TP}$. Note that in unstressed conditions $E_{TA}=E_{TP}=E_{TU}$. We define a water stress term, $\beta(\psi_e)$ (-), where $0\leq\beta\leq 1$, and $\beta$ is a function of the effective matric potential, $\psi_e$. Now, we have an uptake term that would be defined as 

\begin{equation}
s_{v,i}=-E_{TP}\beta(\psi_{e,i})w_i
\end{equation}

and hence

\begin{equation}
s_{v,i}=-E_{TP}\beta(\psi_{e,i})\frac{g(z_i)}{\sum_{j=1}^Ng(z_j)dz_j}
\end{equation}

It follows that the actual transpiration under stressed conditions, $E_{TA}$ (m s$^{-1}$) is given by

\begin{equation}
E_{TA}=\sum_{i=1}^N -s_{v,i} dz_i
\end{equation}

It is also necessary to define a corresponding energy source term, that relates to the amount of energy that is removed by the water uptake. This is given by

\begin{equation}
s_{u,i}=s_{v,i}\rho_l c_{pl}T_i
\end{equation}

One consequence of this, that can be seen from examining Equation \ref{eq: unfrozEbalSource} is that for unfrozen soil conditions, while transpiration removes energy from the soil, it does not change the soil temperature. For frozen soil conditions we assume that $\beta=0$ in order to restrict transpiration from frozen soil layers. This is an assumption that could be explored further in future work. Note, latent heat of vaporization effects are not accounted for here, and are assumed to be accounted for in the land-surface energy balance.

\section{Soil evaporation}

Soil evaporation is calculated in the same way as transpiration, but is only taken from the uppermost cell, i.e., $i=1$. Again we define the potential soil evaporation, $E_{SP}$ (m s$^{-1}$) and actual soil evaporation, $E_{SA}$ (m s$^{-1}$), and the water stress term $\gamma(\psi_e)$ (-) (which does not have to be the same as $\beta(\psi_e)$). Hence we have the sink term given by

\begin{equation}
s_{v,1}=-E_{SP}\frac{\gamma(\psi_{e,1})}{dz_1}
\end{equation}

And total soil evaporation is given by 

\begin{equation}
E_{SA}=-s_{v,1} dz_1 = E_{SP} \gamma(\psi_{e,1})
\end{equation}

The corresponding energy source term

\begin{equation}
s_{u,1}=s_{v,1}\rho_l c_{pl}T_1
\end{equation}

must again be applied. Again, we assume that $\gamma=0$ if the top layer of the soil is frozen.
