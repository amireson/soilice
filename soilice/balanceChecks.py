...
def soilBalanceCheck(model):
    nt,nz=model.thetaL.shape
    ml=model.thetaL*model.dz*model.const['rho_liq']
    mi=model.thetaI*model.dz*model.const['rho_ice']
    ms=np.zeros((nt,nz))+((1-model.pars['thetaS'])*model.dz*model.pars['rho_soil'])
    u=(ml*model.const['cp_liq']+mi*model.const['cp_ice']+ms*model.pars['cp_soil'])*model.T-mi*model.const['lambda_f']
    ml=np.sum(ml,axis=1)
    mi=np.sum(mi,axis=1)
    ms=np.sum(ms,axis=1)
    u=np.sum(u,axis=1)
    ...
    pl.figure()
    pl.subplot(2,1,1)
    pl.plot(t,(qT-qB)*model.const['rho_liq'],'.',label='net mass in')
